    ...
    private static String getChar(int x, int y){
        ...
        for (Person human: Main.all) {
            if (human.getCoords()[0] == x && human.getCoords()[1] == y){
                ...
                if (Main.darkTeam.contains(human)) out = "|" + (AnsyView.ANSI_GREEN + human.getInfo().charAt(0) + AnsyView.ANSI_RESET);
                if (Main.holyTeam.contains(human)) out = "|" + (AnsyView.ANSI_BLUE + human.getInfo().charAt(0) + AnsyView.ANSI_RESET);
                break;
            }
        }
        return out;
